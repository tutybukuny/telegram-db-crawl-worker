...
func (s *serviceImpl) ForwardMessage(ctx context.Context, message *client.Message) error {
	...
	s.ll.Info("received message for forwarding", l.Object("message", message))
	s.limiter.Take()

	return s.forwarder.Forward(ctx, config, message)
}
